...
if ( CheckVersion($DBversion) ) {
    print "Upgrade to $DBversion done (Koha 3.20.1)\n";
    SetVersion ($DBversion);
}
...
if ( CheckVersion($DBversion) ) {
    ...
    print
"Upgrade to $DBversion done (Bug 14298: AutoSelfCheckID user should only be able to access SCO )\n";
    SetVersion($DBversion);
}

$DBversion = "3.20.04.000";
if ( CheckVersion($DBversion) ) {
    print "Upgrade to $DBversion done (Koha 3.20.4)\n";
    SetVersion($DBversion);
}

$DBversion = "3.20.04.001";
if ( CheckVersion($DBversion) ) {
    $dbh->do(q{
        INSERT IGNORE INTO systempreferences (variable,value,options,explanation,type)
        VALUES
