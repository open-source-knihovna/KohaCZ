...
if ( CheckVersion($DBversion) ) {
    ...
    $dbh->do(q{
        INSERT IGNORE INTO systempreferences (variable,value,explanation,options,type) VALUES
        ('NotifyToReturnItemWhenHoldIsPlaced', '0', 'If ON, notifies the patrons to return an item when ever a hold is placed on it', NULL, 'YesNo');
    });

    $dbh->do(q{
        INSERT IGNORE INTO systempreferences (variable,value,explanation,options,type) VALUES
        ('NotifyToReturnItemFromLibrary', 'ItemHomeLibrary', 'Restricts what library to take into consi deration when notifying patrons to return items on hold', 'RequestorLibrary|ItemHomeLibrary|AnyLibrary', 'Choice');
    });
    print "Bug 17509: Add NotifyToReturnItemWhenHoldIsPlaced system preferences\n";
    ...
    print "Bug 17509: Add notification template to notify that a hold was placed on checked out item\n";
...
$DBversion = '17.11.02.000';
if( CheckVersion( $DBversion ) ) {
    SetVersion( $DBversion );
    print "Upgrade to $DBversion done (17.11.02 release)\n";
}

$DBversion = '17.11.03.000';
if( CheckVersion( $DBversion ) ) {
    SetVersion( $DBversion );
    print "Upgrade to $DBversion done (17.11.03 release)\n";
}

$DBversion = '17.11.04.000';
if( CheckVersion( $DBversion ) ) {
    SetVersion( $DBversion );
    print "Upgrade to $DBversion done (17.11.04 release)\n";
}

$DBversion = '17.11.05.000';
if( CheckVersion( $DBversion ) ) {
    SetVersion( $DBversion );
    print "Upgrade to $DBversion done (17.11.05 release)\n";
}

$DBversion = '17.11.06.000';
if( CheckVersion( $DBversion ) ) {
    SetVersion( $DBversion );
    print "Upgrade to $DBversion done (17.11.06 release)\n";
}
