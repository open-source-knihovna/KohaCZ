...
sub GetLoanLength {
    ...
    my $sth = $dbh->prepare(qq{
        SELECT issuelength, maxissuelength, lengthunit, renewalperiod, reserved_renew_period
        FROM issuingrules
        WHERE   categorycode=?
            AND itemtype=?
            AND branchcode=?
...
sub CanBookBeRenewed {
    ...
    if (defined $issuing_rule && $issuing_rule->maxissuelength) {
        $max_date_due = $itemissue->{issuedate}->clone->add( $issuing_rule->lengthunit => $issuing_rule->maxissuelength );
    ...
    return ( 0, "too_many" )
        if not $issuing_rule or (
            $issuing_rule->renewalsallowed <= $itemissue->{renewals}
            or (defined $max_date_due && $max_date_due <= $itemissue->{date_due} ) );
...
sub AddRenewal {
    ...
    unless ($datedue) {
        ...
        $datedue =  CalcDateDue($datedue, $itemtype, _GetCircControlBranch($item, $borrower), $borrower, 'is a renewal', $resfound, $issuedata->{issuedate});
    }
...
sub CalcDateDue {
    my ( $startdate, $itemtype, $branch, $borrower, $isrenewal, $isreserved, $issuedate ) = @_;
