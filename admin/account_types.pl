...
if ( $op eq 'add_form' ) {
    ...
    my $can_be_added_manually = $input->param('can_be_added_manually');
    my $default_amount;
    if ($type eq "debit") {
        $default_amount = $input->param('default_amount');
    }
    ...
    if ($type_code) {
        ...
        my $account_type;
        if($type eq "debit") {
            $account_type = Koha::Account::DebitType->new(
            {   type_code    => $type_code,
            ...
            $account_type = Koha::Account::CreditType->new(
            {   type_code    => $type_code,
                description   => $description,
                can_be_added_manually => $can_be_added_manually,
            });
